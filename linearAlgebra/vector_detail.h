...
template <typename T, std::size_t N>
T sum(const VectorClass<T, N>& vector)
{
...
template <typename T,
          typename = Enable_if<Floating_Point<T>()> >
std::tuple<VectorClass<T, 3>, VectorClass<T, 3>,  VectorClass<T, 3>>
...
template <typename T>
auto AcuteAngle(const VectorClass<T, 2>& a, const VectorClass<T, 2>& b)
{
    auto dot = Dot(a, b);
    auto crs = Cross(a, b);
...
template <typename T>
auto AcuteAngle(const VectorClass<T, 3>& a, const VectorClass<T, 3>& b)
{
...
template <typename T, std::size_t N>
VectorClass<T, N> ones()
{
    VectorClass<T, N> ones(T(1));
    return ones;
}

template <typename T, std::size_t N>
VectorClass<T, N> linspace(T start, T end)
{
    VectorClass<T, N> array;
